...
app.put('/storage/', (req, res) => {
  for (let i = 0;i<albums.length;i++)
  {
    if (albums[i].id == req.body.id)
      {
        albums[i].title = req.body.title;
      }
  }
...
{
  console.log("WORKED");
  albums.push({"title":req.body.title,"id":id});
  res.json({"title":req.body.title,"id":id});
  id++;
});
